...
def install_app(full_filepath):
    ...
    r = requests.post('https://192.168.38.110:9999/rest/app',
...
# Get session cookies
try:
    s.get('https://localhost:9999/login')
except Exception as e:
...
try:
    data = {"uri": "https://github.com/timfrazier1/AdvSimPlaybooks.git", 
            ...
            "read_only": "on",
            "save": "true"}
    repo_resp = s.post('https://localhost:9999/scm/', data)
...
# Grab token
try:
    token_response = s.get('https://localhost:9999/rest/ph_user/2/token')
...
try:
    s.post('https://localhost:9999/rest/ph_user/2', json={"allowed_ips": ["any"], "default_label": "advsim_test"})
except Exception as e:
    print "Unable to update allowed IPs to any: " + str(e)

try:
    s.post('https://localhost:9999/rest/asset', json={"configuration": {"verify_cert": True,
...
try:
    s.post('https://localhost:9999/rest/container', json={"label": "events", "name": "Example Container"})
except Exception as e:
    print "Example container POST failed: " + str(e)
try:
    art_app_response = s.get('https://localhost:9999/rest/app?_filter_name__contains=%22Atomic%22')
...
#f.close()
try:
    s.post('https://localhost:9999/rest/action_run', json={"action": "test connectivity", "container_id": 1, "name": "art_test_connectivity", "targets": [{"assets": ["art_main_repo"], "parameters": [], "app_id": art_app_id}]})
except Exception as e:
...
time.sleep(120)
try:
    s.post('https://localhost:9999/rest/asset', json={"name": "winrm_dect_lab", "product_name": "Windows Remote Management", "product_vendor": "Microsoft", "configuration": {"username": "vagrant", "domain": "", "endpoint": "192.168.38.104", "verify_server_cert": False, "default_port": "5985", "default_protocol": "http", "password": "vagrant", "transport": "ntlm"}})
    s.post('https://localhost:9999/rest/asset', json={"name": "splunk_dect_lab", "product_vendor": "Splunk Inc.", "product_name": "Splunk Enterprise", "configuration": {"username": "admin", "max_container": 100, "ingest": {"container_label": "splunk_events", "start_time_epoch_utc": ""}, "retry_count": "3", "verify_server_cert": False, "device": "192.168.38.105", "timezone": "UTC", "password": "changeme", "port": "8089"}})
    s.post('https://localhost:9999/rest/asset', json={"name": "dect_lab_logger", "product_vendor": "Generic", "product_name": "SSH", "configuration": {"username": "vagrant", "password": "vagrant", "root": False, "test_device": "192.168.38.105"}})
except Exception as e:
    print "POST assets failed: " + str(e)

try:
    repo_response = s.get('https://localhost:9999/rest/scm?_filter_name=%22AdvSim%22')
    repo_id = repo_response.json()['data'][0]['id']

    s.post('https://localhost:9999/rest/scm/' + str(repo_id), json= {"pull": True, "force": True})
except Exception as e:
    print "GET and POST repo info failed: " + str(e)

try:
    playbook_response = s.get('https://localhost:9999/rest/playbook?_filter_name=%22Modular%20Simulation%22')
    playbook_id = playbook_response.json()['data'][0]['id']

    s.post('https://localhost:9999/rest/playbook/' + str(playbook_id), json = {"active": True, "cancel_runs": True})
except Exception as e:
    print "GET and POST playbook info failed: " + str(e)


try:
    winrm_app_response = s.get('https://localhost:9999/rest/app?_filter_name__contains=%22Remote%22')
    winrm_app_id = winrm_app_response.json()['data'][0]['id']
except Exception as e:
    print "Get WinRM App ID failed: " + str(e)
try:
    s.post('https://localhost:9999/rest/action_run', json={"action": "run script", "container_id": 1,
